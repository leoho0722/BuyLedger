...
App 使用兩把 API key，皆透過 `Config.xcconfig` 注入：
...
注入鏈路說明 (兩把 key 相同)：`Config.xcconfig` → pbxproj 的 `baseConfigurationReference` 掛到 app target 的 Debug + Release config → `Info.plist` 內以 `$(EXCHANGE_RATE_API_KEY)` / `$(OLLAMA_API_KEY)` 引用 build setting，build 時被注入 → `APIKeyProvider.liveValue` 從 `Bundle.main.infoDictionary` 讀回。
...
- **Schema 版本化**：`Core/Persistence/BuyLedgerSchema.swift` 以 `VersionedSchema` (floor `BuyLedgerSchemaV7` → target `BuyLedgerSchemaV11`，floor 以下版本已移除) + `BuyLedgerMigrationPlan` 管理遷移。新增欄位／表走 lightweight、改型別走 custom dump-and-restore；改動流程的硬規則見 [`CLAUDE.md › SwiftData Schema 與 Migration`](CLAUDE.md#swiftdata-schema-與-migration)。
...
`OllamaClient` (`Core/Networking/`) 串接 Ollama Cloud chat streaming (`POST https://ollama.com/api/chat`)，供 `Features/AISummary/` 在訂單詳情串流產生 Markdown 商品明細總結；缺金鑰或服務錯誤時進入 failed 狀態並提供重試，面板關閉時取消串流。
