...
///
/// 旗標關閉 (預設) 時直接顯示 ``RootView``，
/// 行為與導入雲端同步前完全一致 (本機 SwiftData、不顯示登入、不存取 Firestore)
...
/// 雲端登入閘門：未登入時顯示 ``CloudSignInView``、登入後顯示 ``RootView``
/// 僅在旗標開啟時使用
    ...
    var body: some View {
        ...
        .task(id: auth.uid) {
            ...
            let newEngine = CloudSyncEngine(
                ...
                now: { date.now },
                connectivity: { NetworkPathMonitor.liveValue.connectivity() }
            )
