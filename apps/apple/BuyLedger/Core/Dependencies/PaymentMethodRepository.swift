...
struct PaymentMethodRepository: Sendable {
    ...
    ///
    /// 參數依序為 (名稱, isCardless, isBankTransfer, isCashOnDelivery)。若名稱已存在，會以新傳入的旗標覆寫 (用來在使用者第二次新增同名項目時修正先前勾選)
    var addPaymentMethod: @Sendable (String, Bool, Bool, Bool) async throws -> Void
