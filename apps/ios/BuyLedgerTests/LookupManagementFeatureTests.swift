...
struct LookupManagementFeatureTests {
    ...
    @Test func reconciliationStatusAddConfirmedAppendsItem() async {
        await Self.withIsolatedCatalog {
            ...
            await store.send(
                .addConfirmed(
                    name: "待對帳", isCardless: false, isBankTransfer: false, isCashOnDelivery: false)
            ) {
    ...
    @Test func addConfirmedWritesThroughToTheSharedCatalogFromAStandaloneContainer() async {
        await Self.withIsolatedCatalog {
            ...
            await store.send(
                .addConfirmed(
                    name: "手工藝品", isCardless: false, isBankTransfer: false, isCashOnDelivery: false)
            ) {
    ...
    @Test func editConfirmedRenamesPaymentMethodAndClearsFlag() async {
        await Self.withIsolatedCatalog {
            ...
            await store.send(
                .editConfirmed(
                    originalName: "匯款", name: "銀行匯款", isCardless: false, isBankTransfer: false,
    ...
    @Test func editConfirmedKeepsNameAndUpdatesFlags() async {
        await Self.withIsolatedCatalog {
            ...
            await store.send(
                .editConfirmed(
                    originalName: "銀行匯款", name: "銀行匯款", isCardless: false, isBankTransfer: true,
                    isCashOnDelivery: false))
            // 目錄變更會在此接收點反映。
            await store.receive(\.paymentMethodEditPrepared) {
                $0.$catalog.withLock {
                    $0.paymentMethods = [
                        PaymentMethodInfo(
                            name: "銀行匯款", isCardless: false, isBankTransfer: true,
                            isCashOnDelivery: false)
                    ]
                }
            }
    ...
    @Test func editWithUnchangedFlagsRenamesWithoutRetroactiveConfirmation() async {
        await Self.withIsolatedCatalog {
            ...
            state.$catalog.withLock {
                $0.paymentMethods = [
                    PaymentMethodInfo(
                        name: "信用卡", isCardless: false, isBankTransfer: false,
            ...
            await store.send(
                .editConfirmed(
                    originalName: "信用卡", name: "信用卡（新）", isCardless: false, isBankTransfer: false,
                    isCashOnDelivery: true))
            await store.receive(\.paymentMethodEditPrepared) {
                $0.$catalog.withLock {
                    $0.paymentMethods = [
                        PaymentMethodInfo(
                            name: "信用卡（新）", isCardless: false, isBankTransfer: false,
                            isCashOnDelivery: true)
                    ]
                }
            }
