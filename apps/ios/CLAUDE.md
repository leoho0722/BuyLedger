...
- **收鍵盤有四條路徑，且不得以全域攔截加排除清單實作**：一般鍵盤的 return 鍵、數字鍵盤的鍵盤工具列、`scrollDismissesKeyboard(.interactively)`、以及背景點擊 `dismissKeyboardOnBackgroundTap(focus:)` (`Shared/Keyboard/`)。每個有輸入的畫面至少要有兩條。
    - **背景點擊由背景層自己承接、不掛 window 級手勢**：舊實作對所有觸控先攔一次、再靠黑名單排除互動元件，而黑名單永遠追不上系統新增的 view 型別 (曾誤觸貼上／選取等系統 action)。改由 `.background { Color.clear.contentShape(.rect).onTapGesture { ... } }` 承接——互動元件天生不在背景層上，不需要任何排除清單。
    - **收鍵盤一律把焦點設為 `nil`／`false`，不呼叫 UIKit 的 `endEditing`**：讓收鍵盤與焦點成為同一個狀態的兩面，不會出現「鍵盤已收但焦點仍指向某欄位」。
    - 用系統 `.searchable` 的畫面毋須加掛背景點擊——其 Cancel 鈕與捲動收合由系統提供。
    - 「點系統文字選單不收鍵盤」由 `BuyLedgerUITests/KeyboardDismissTests` 守住 (該行為依賴系統選單的 view 階層，並非公開契約)。
