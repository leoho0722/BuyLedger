...
///
/// 壞值 (缺值、對不到列舉、日期解析失敗) 一律退回預設值並印一行 `[BuyLedger][UITest]` 警告，絕不 crash
/// 不讀 `UserDefaults`，只認傳入的 `arguments`，讓 ``init(arguments:)`` 可被單元測試直接呼叫
struct BLUITestConfiguration: Equatable, Sendable {
    ...
    /// 是否開啟 AI 商品明細總結 (`-BLUITestAiSummary`)；預設關閉，開啟後 AI 走固定輸出的替身
    let useAiSummary: Bool

    // MARK: - Static Properties

    /// 由本次 process 的啟動參數建立的設定
    ...
    static let defaultReferenceDate: Date = {
        ...
        let components = DateComponents(year: 2026, month: 4, day: 30, hour: 0, minute: 0, second: 0)
        guard let date = calendar.date(from: components) else {
            // 固定常數理應必定可建構；建不出來代表前提壞掉，寧可炸也不要靜默給錯誤時間
    ...
    /// 從啟動參數陣列解析設定
    /// - Parameter arguments: 完整的啟動參數 (通常是 `ProcessInfo.processInfo.arguments`)
...
private extension BLUITestConfiguration {
    ...
    /// 解析可對應到 `String` rawValue 列舉的旗標；缺值或對不到 case 時回 `nil` 讓呼叫端套預設值
    /// - Parameters:
    ...
    ///
    /// 幣別代號的正規形式為大寫，故解析時一併轉大寫 (`twd` 與 `TWD` 等價)
    /// - Parameter arguments: 完整的啟動參數陣列
    ...
    /// 解析 `-BLUITestMonthlyGoal`；非整數或負數時回 `nil` (0 是有效值，代表未設定目標)
    /// - Parameter arguments: 完整的啟動參數陣列
    ...
    /// 取旗標後面那個值；缺值時警告並回 `nil`，旗標重複時採最後一次 (對齊「共用基底 + 個案覆寫 append」的用法)
    /// - Parameters:
    ///   - flag: 要取值的旗標名
    ///   - arguments: 完整的啟動參數陣列
    ...
    /// ISO8601 解析：完整時間 (帶不帶小數秒皆可) 與純日期 (視為當日 00:00Z) 都吃
