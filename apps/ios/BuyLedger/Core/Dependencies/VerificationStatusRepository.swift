...
struct VerificationStatusRepository: Sendable {
    ...
    var addVerificationStatus: @Sendable (_ rawName: String) async throws -> Void

    /// 刪除指定名稱的對帳狀態；不存在視為 no-op
    /// - Parameter name: 要刪除的名稱
    var removeVerificationStatus: @Sendable (_ name: String) async throws -> Void

    /// 把指定對帳狀態更名 (舊名 → 新名)。caller 負責把訂單表的 cascade 一併處理；本方法只更新主檔
    /// - Parameters:
    ///   - oldName: 舊名稱
    ///   - newName: 新名稱
    var renameVerificationStatus: @Sendable (_ oldName: String, _ newName: String) async throws -> Void
}
...
private extension VerificationStatusRepository {
    ...
    static func makePersistence(container: ModelContainer) async -> VerificationStatusPersistence {
        await MainActor.run {
