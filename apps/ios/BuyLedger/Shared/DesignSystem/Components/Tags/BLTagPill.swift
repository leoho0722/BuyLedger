...
///
/// 視覺沿用 ``BLStatusPill`` 的 neutral 外觀 (灰底、無狀態指示點)，常用於商品類別等分類標記
///
/// 圖示置於膠囊外、與膠囊垂直置中對齊；膠囊文字過長時換行成多行、膠囊高度隨之增加 (不截斷、也不撐爆容器寬度)
struct BLTagPill: View {
    ...
    var body: some View {
        HStack(alignment: .center, spacing: BLSpacing.extraSmall) {
            ...
            // 改用 `fixedSize(horizontal: false, vertical: true)` 讓膠囊隨文字換行增高，不截斷也不溢出
