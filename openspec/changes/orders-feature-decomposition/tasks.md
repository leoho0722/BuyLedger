...
- [ ] 5.4 在實機以訂單編輯表單連續輸入數字欄位確認無新增卡頓 (驗證草稿型別的可觀察標註確實生效，此問題的症狀是變卡而非功能錯誤，只靠測試不會發現)。驗證：人工確認，並比對介面測試中訂單編輯相關案例的執行時間無明顯劣化
    - 留白說明：本輪 QA／Style 修正僅有模擬器可用、無實機環境，未能執行此項人工確認；`OrderDraft` 已確認標註 `@ObservableState` (結構性前提成立)，但「連續輸入無新增卡頓」的實機感受仍待補做
