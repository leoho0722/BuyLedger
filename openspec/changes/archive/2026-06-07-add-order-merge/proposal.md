...
- **訂單合併流程**：
  ...
  - 候選選擇：sheet 列出可合併的其他訂單 (限同幣別、排除「已合併」與「已取消」)，顯示客戶名稱、訂購日期與客戶實付，支援搜尋；僅列與主訂單同客戶名稱的訂單 (不支援跨客戶合併)。
...
- Affected code:
  ...
  - Modified:
    ...
    - BuyLedger/BuyLedger/Features/Orders/OrdersFeature.swift、BuyLedger/BuyLedger/Features/Orders/OrdersView.swift、BuyLedger/BuyLedger/Features/Orders/OrdersCompactView.swift (篩選、context menu 入口、sheet 掛載)
    ...
    - BuyLedger/BuyLedger/Features/Orders/Components/OrderRowView.swift、BuyLedger/BuyLedger/Features/Orders/Components/OrderFilterSheet.swift、BuyLedger/BuyLedger/Features/Orders/Components/OrderDetailView.swift (多類別/多開團呈現；詳情頁合併入口)
