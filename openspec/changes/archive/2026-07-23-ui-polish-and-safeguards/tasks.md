...
- [x] 4.4 [P] 移除分頁選擇的持久化與啟動還原，App 重啟固定顯示總覽分頁 (原「恢復上次分頁」經實機驗收判定不符實際需求，已反轉提案)。滿足 The app launches on the overview tab。行為：切到任一分頁後重啟，一律回到總覽。驗證：以 TestStore 斷言啟動不改變預設分頁；全專案搜尋確認分頁欄位與讀寫無殘留。
