...
- [x] 3.2 依決策〈不設全域 tint，自訂元件強調色改走系統動態藍〉，強調色資源檔維持不填值、RootView 不設 `.tint`，BLPalette 的強調色改為系統動態藍。滿足 The app does not force a global tint (原「強調色單一來源＋根層設定」經實機驗收判定不符實際需求，已修正提案)。行為：系統元件維持系統預設 accent，自訂元件由色盤取系統動態藍。驗證：實機確認整個 App 未被強制上色、外觀與未設定 tint 前一致。
