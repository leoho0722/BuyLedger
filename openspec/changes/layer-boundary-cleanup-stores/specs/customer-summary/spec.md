...
The orders the screen aggregates SHALL reach it as a read-only projection held in the customers feature's own state and written only by the root feature. The screen SHALL NOT read the orders feature's state directly. Selecting a customer SHALL be expressed as a delegate action that the root feature forwards to its existing customer-selection handling, preserving that handling's existing ordering, in particular clearing the hosting navigation path before switching tabs.
...
#### Scenario: The screen reads a projection rather than another feature's state

- **WHEN** the customers screen obtains the orders it aggregates
- **THEN** it reads the projection in its own feature's state, which the root feature is the sole writer of

#### Scenario: Selecting a customer preserves the existing navigation ordering

- **WHEN** the user selects a customer from the customers screen
- **THEN** the delegate is forwarded to the existing customer-selection handling, which clears the hosting navigation path before switching tabs, so the split-view navigation on large screens does not fault
