...
Manage photos attached to an order from the order edit form: pick up to 5 images via the native photo picker, normalize them before storage, view them full screen with swipe navigation, delete individual photos, and persist them with the order record in SwiftData so they survive app restarts and schema migrations.
