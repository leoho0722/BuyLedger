...
private extension OrderDetailView {
    ...
    func profitCard(summary: OrderSummary, palette: BLPalette) -> some View {
        BLCard {
            VStack(alignment: .leading, spacing: BLSpacing.large) {
                ...
                HStack {
                    metric("總收款", value: OrderFormatters.twd(summary.revenue), palette: palette)
                    Spacer()
                    metric("總成本", value: OrderFormatters.twd(summary.totalCost), palette: palette)
                    Spacer()
                    metric("手續費", value: OrderFormatters.twd(summary.fees), palette: palette)
                }
            }
            .frame(maxWidth: .infinity, alignment: .leading)
        }
    ...
    ///   - palette: 目前外觀使用的色盤。
    /// - Returns: 成本條列 view。
    func breakdownBarRow(_ component: OrderCostComponent, total: Double, palette: BLPalette) -> some View {
...
private extension OrderDetailView {
    ...
    func costComponents(palette: BLPalette) -> [OrderCostComponent] {
        [
            OrderCostComponent(title: "商品金額", value: order.itemCost, color: palette.accent),
            OrderCostComponent(title: "國內運費", value: order.domesticShipping, color: palette.teal),
            OrderCostComponent(title: "國際集運", value: order.internationalShipping, color: palette.purple),
            OrderCostComponent(title: "手續費", value: order.summary.fees, color: palette.orange),
        ]
        .filter { $0.value > 0 }
    }
...
#Preview("訂單詳情 · 窄欄") {
    NavigationStack {
        OrderDetailView(order: LedgerOrder.sampleOrders[0])
    }
}

#Preview("訂單詳情 · 寬欄") {
    OrderDetailView(order: LedgerOrder.sampleOrders[0], layout: .wide)
