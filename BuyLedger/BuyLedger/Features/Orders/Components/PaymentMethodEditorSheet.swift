...
///
/// SwiftUI 的 `.alert` actions builder 只支援 `Button`／`TextField`，無法塞入 `Toggle`；而「是否為無卡類／銀行匯款類」必須在當下決定，所以付款方式的新增與編輯流程改採獨立 sheet，將名稱輸入與兩個旗標切換放在同一張表單，確認後一次帶出 `(name, isCardless, isBankTransfer)` 給 caller。
///
/// 編輯既有付款方式時，caller 透過 `initialName` / `initialIsCardless` / `initialIsBankTransfer` 帶入原有資料，並把 `title` 設為「編輯付款方式」、`submitTitle` 設為「儲存」。
///
...
struct PaymentMethodEditorSheet: View {
    ...
    /// 使用者按下儲存時的 callback；caller 拿到 `(name, isCardless, isBankTransfer)` 後負責寫入主檔。
    let onSubmit: (_ name: String, _ isCardless: Bool, _ isBankTransfer: Bool) -> Void
    ...
    ///   - onSubmit: 確認時的 callback，帶出 `(name, isCardless, isBankTransfer)`。
    init(
        title: String,
        message: String,
        ...
        onSubmit: @escaping (_ name: String, _ isCardless: Bool, _ isBankTransfer: Bool) -> Void
    ) {
        self.title = title
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button(submitTitle) {
                        ...
                        onSubmit(trimmed, draftIsCardless, draftIsBankTransfer)
                        dismiss()
                    }
